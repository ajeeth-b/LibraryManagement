...
function update_table_rows(start, end){
	...
	for(;start<=end; start++){
		...
		if (data != undefined){
			...
			row.insertCell(3).innerHTML =  "<a href='#' onclick=return_book("+data.id+ ",'" + 'd'+start +"')>Return</a>"
		}
	}
	update_pagination();
}
...
function update_local_db(offset, update_table_later=false){
	...
	url = '/api/library/members/'+member_id+'/books?offset='+offset+'&per-page='+row_per_page;
